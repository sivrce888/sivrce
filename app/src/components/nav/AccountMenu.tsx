...
export function AccountMenu({
  ...
  const identity = (
    <div className="flex items-center gap-3 border-b border-sv-ink/8 px-4 py-3">
      <UserAvatar name={user?.name} image={user?.image} gradient={user?.avatarStyle} color={user?.avatarColor} icon={user?.avatarIcon} size={40} />
  ...
  return (
    <details className="relative">
      ...
      >
        <UserAvatar name={user?.name} image={user?.image} gradient={user?.avatarStyle} color={user?.avatarColor} icon={user?.avatarIcon} size={28} />
      </summary>
