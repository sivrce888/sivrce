...
/**
 * Building click panel — sale / rent / daily tabs + listing links.
 */
      ...
      <header className="shrink-0 border-b border-sv-ink/6 p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            ...
            onClick={onClose}
            aria-label="დახურვა"
            className="grid h-9 w-9 place-items-center rounded-control text-sv-ink/40 transition hover:bg-sv-cloud hover:text-sv-ink"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
          ...
          {TABS.map((t) => {
            ...
            return (
              <button
                ...
                style={active ? { background: t.color ?? '#2E6BFF' } : undefined}
              >
                {t.label}
              </button>
            )
          ...
          list.map((l) => {
            ...
            return (
              <li key={l.id}>
                <Link
                  href={`/listing/${l.id}`}
                  className="flex gap-3 rounded-module border border-sv-ink/6 bg-sv-cloud/60 p-2.5 transition hover:border-sv-blue/30 hover:bg-white"
                >
                  ...
                  <div className="min-w-0 flex-1 py-0.5">
                    ...
                    <div className="text-[11px] font-semibold text-sv-ink/40">
