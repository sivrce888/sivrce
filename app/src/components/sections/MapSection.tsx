...
/* Feature copy lives in CMS blocks (home.map.f1–f4) — localized per lang. */
const FEATURES = [
  { icon: MousePointerClick, titleKey: 'home.map.f1.title', textKey: 'home.map.f1.text' },
  { icon: Building2, titleKey: 'home.map.f2.title', textKey: 'home.map.f2.text' },
  { icon: BarChart3, titleKey: 'home.map.f3.title', textKey: 'home.map.f3.text' },
  { icon: Layers, titleKey: 'home.map.f4.title', textKey: 'home.map.f4.text' },
] as const

/** Zero-JS map preview — LCP: no MapLibre on homepage. Real map lives on /map. */
function MapPreviewCard({ badge }: { badge: string }) {
  return (
              ...
              {FEATURES.map((f, i) => (
                <Reveal key={f.titleKey} delay={0.1 + i * 0.08}>
                  <div className="group flex gap-5 rounded-module border border-white/[0.07] bg-white/[0.03] p-5 transition-all duration-500 hover:border-sv-blue/40 hover:bg-white/[0.06]">
                    ...
                    <div>
                      <h3 className="text-[16px] font-extrabold text-white">{b(f.titleKey)}</h3>
                      <p className="mt-1 text-[14px] font-medium leading-relaxed text-white/55">{b(f.textKey)}</p>
                    </div>
                  </div>
                </Reveal>
              ))}
