      ...
      <div className="mx-auto max-w-[1440px] px-5 md:px-10">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          {items.map((p, i) => {
            ...
            return (
              <Reveal key={p.slug} delay={i * 0.12} className="h-full">
                <LocalizedLink href={`/projects/${p.slug}`} className="block h-full">
                  <article className="group flex h-full cursor-pointer flex-col overflow-hidden rounded-card border border-sv-ink/[0.06] bg-sv-surface shadow-card transition-all duration-500 hover:-translate-y-2 hover:shadow-card-hover">
                      ...
                      <div className="absolute bottom-4 left-5 right-5 flex items-end justify-between">
                        <div>
                          ...
                          <p className="flex items-center gap-1.5 text-[13px] font-bold text-white/80">
                            <BadgeCheck className="h-4 w-4 text-sv-success" /> {devName}
                          </p>
                        </div>
                    ...
                    <div className="mt-auto flex flex-wrap items-center gap-x-6 gap-y-3 p-5">
                      ...
                      <span className="ml-auto text-[16px] font-black text-sv-blue">
                        {p.priceFromM2}<span className="text-[12px] font-bold text-sv-ink/60"> /მ²-დან</span>
                      </span>
                    </div>
