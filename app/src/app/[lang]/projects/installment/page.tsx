...
export default async function ProjectsInstallmentPage({ params }: PageProps) {
  ...
  const projects = (await projectsLive()).filter(SUB_HUBS.installment.filter)
  return <ProjectHub loc={loc} c={PROJECT_HUBS.installment[loc]} projects={projects} basePath={SUB_HUBS.installment.path} />
}
