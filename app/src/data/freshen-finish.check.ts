...
console.assert(stale.done === 100 && stale.finish === 'ჩაბარებული (2024)', 'stale completes')
...
const already = freshenFinish({ done: 100, finish: `ჩაბარებული (${YEAR - 1})` })
console.assert(already.finish.startsWith('ჩაბარებული'), 'already-delivered untouched')

// Legacy DB rows still carry the pre-2026-09 'გადაცემულია' mark — parsed, never corrupted.
const legacy = freshenFinish({ done: 40, finish: 'გადაცემულია (2024)' })
console.assert(legacy.finish === 'გადაცემულია (2024)', 'legacy mark untouched')
console.assert(isDelivered({ done: 40, finish: 'გადაცემულია (2021)' }), 'legacy mark counts as delivered')
...
console.assert(isDelivered({ done: 100, finish: 'ჩაბარებული (2021)' }), 'explicit delivery')
...
const leaks = PROJECTS.filter((p) => {
  if (p.done >= 100 || p.finish.startsWith('ჩაბარებული')) return false
...
console.log(`freshen-finish: ${PROJECTS.length} projects clean, 9 unit cases ✓`)
