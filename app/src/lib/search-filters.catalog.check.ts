...
// Country scoping (host/market layer decides default; search accepts market ISOs only).
assert.equal(parseSearchParams(new URLSearchParams()).country, undefined)
...
assert.equal(parseSearchParams(new URLSearchParams('country=FR')).country, 'FR')
assert.equal(parseSearchParams(new URLSearchParams('country=XX')).country, undefined)
