...
// 6th cron (dedupe-fraud, nightly) is deliberate — trust sweep feeding admin
// Moderation. Revert the cap if you'd rather fold it into an existing job.
assert.ok(vercel.crons.length <= 6, "extra crons = extra invocations")
...
lock("src/app/[lang]/layout.tsx", ["LITE_BOOT", 'id="lite-boot"', "slogan: BRAND.tagline.ka", "cdn.sivrce.ge"])
