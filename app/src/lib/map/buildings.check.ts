...
import { DEVELOPERS, projectCode } from '@/data/professionals'
import { readFileSync, writeFileSync } from 'node:fs'
import { fileURLToPath } from 'node:url'
import type { BuildingCatalogEntry } from '@/data/buildings'

/**
 * lib/map/map-catalog.gen.ts — what map pins read from the catalogs. Importing
 * data/buildings + data/professionals for it shipped the world project catalog
 * (~390 KB gz: galleries, 3-locale copy) to every map visit. Tuple rows: 970 ×
 * 16 repeated keys was a third of the bytes.
 */
function mapCatalogSource(): string {
  // First match wins, as in getDeveloper (Array.find) — a duplicate slug must not flip.
  const devName: Record<string, string> = {}
  for (const d of DEVELOPERS) devName[d.slug] ??= d.name.ka
  const code = Object.fromEntries(PROJECTS.map((p) => [p.slug, projectCode(p)]))
  const rows = BUILDINGS.map((b) => [
    b.slug, b.code, b.name, b.nameEn, b.address, b.city, b.district,
    b.coords.lat, b.coords.lng, b.buildingNumber, b.img, b.floors, b.status,
    b.developerSlug ?? null, b.projectSlug ?? null, b.yearBuilt ?? null, b.rating ?? null,
  ])
  return `/* GENERATED by \`npx tsx src/lib/map/buildings.check.ts --write\` — do not edit. Drift-locked by the same check. */
export type MapCatalogRow = readonly [
  slug: string, code: string, name: string, nameEn: string, address: string, city: string,
  district: string, lat: number, lng: number, buildingNumber: string, img: string,
  floors: number, status: 'ready' | 'construction', developerSlug: string | null,
  projectSlug: string | null, yearBuilt: number | null, rating: number | null,
]

export const MAP_CATALOG_ROWS: readonly MapCatalogRow[] = ${JSON.stringify(rows)}

export const MAP_DEV_NAME_KA: Readonly<Record<string, string>> = ${JSON.stringify(devName)}

export const MAP_PROJECT_CODE: Readonly<Record<string, string>> = ${JSON.stringify(code)}
`
}

const pick = (b: BuildingCatalogEntry) => {
  const { slug, code, name, nameEn, address, city, district, coords, buildingNumber, img,
    floors, status, developerSlug, projectSlug, yearBuilt, rating } = b
  return Object.fromEntries(
    Object.entries({ slug, code, name, nameEn, address, city, district, coords, buildingNumber,
      img, floors, status, developerSlug, projectSlug, yearBuilt, rating }).filter(([, v]) => v != null),
  )
}
...
async function main() {
  ...
  // The map-catalog leaf must mirror the catalogs byte for byte, or pins drift.
  const genPath = fileURLToPath(new URL('./map-catalog.gen.ts', import.meta.url))
  if (process.argv.includes('--write')) {
    writeFileSync(genPath, mapCatalogSource())
    console.log(`map-catalog.gen.ts: ${BUILDINGS.length} buildings, ${PROJECTS.length} projects`)
    return
  }
  assert.equal(
    readFileSync(genPath, 'utf8'),
    mapCatalogSource(),
    'map-catalog.gen.ts drifted — run: npx tsx src/lib/map/buildings.check.ts --write',
  )
  // …and the decoder must hand buildings.ts the catalog's own values.
  assert.deepEqual(MAP_CATALOG[0], {
    ...pick(BUILDINGS[0]!),
    coords: { lat: BUILDINGS[0]!.coords.lat, lng: BUILDINGS[0]!.coords.lng },
  })
  const base = {
    img: '/x.webp',
    images: ['/x.webp'],
