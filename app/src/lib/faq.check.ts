...
assert.ok(faqMatch('Welche Kaufnebenkosten Georgien Notar Register', 'de'), 'de: georgia fees')
assert.ok(faqMatch('Dorfhaus Grundstück Agrarland Bodenkategorie', 'de'), 'de: agrarland trap')
assert.ok(faqMatch('Was ist der Energieausweis', 'de'), 'de: energieausweis')
assert.ok(faqMatch('WEG Hausgeld Erhaltungsrücklage Sonderumlage', 'de'), 'de: weg hausgeld')
assert.ok(faqMatch('Wie hoch Kaution Berlin Kaltmiete', 'de'), 'de: kaution')
assert.ok(faqMatch('Mietspiegel Mietpreisbremse Vergleichsmiete', 'de'), 'de: mietspiegel')
assert.ok(faqMatch('SCHUFA Wohnung Einkommensnachweise Bürgschaft', 'de'), 'de: schufa')
assert.ok(faqMatch('KfW Finanzierung Eigenkapital Effizienzhaus', 'de'), 'de: kfw')
assert.ok(faqMatch('KI-Preischeck Punktzahl Verhandlungsargument', 'de'), 'de: ki score')
