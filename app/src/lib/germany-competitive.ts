...
export const GERMANY_PLAYERS: readonly Player[] = [
  {
    ...
    cells: {
      provenance: {
        score: 96,
        note: '8 official DE sources fused (ALKIS, StEP Wohnen 2040, BORIS, B-Pläne, Mietspiegel, Statistik BB, Destatis, Handelsregister) with IntelFact/IntelEvidence provenance + refresh cadence.',
        evidence: 'src/lib/intel/core.ts',
      },
      costTransparency: {
        score: 99,
        note: 'buyerCostBreakdown on the DE hub and every DE sale listing (Grunderwerbsteuer 16 states + Makler 3.57% + notary + Grundbuch), EUR-native.',
        evidence: 'src/lib/countries/de.ts',
      },
      rentalRules: {
        score: 97,
        note: 'Kaution cap (§551) and Bestellerprinzip on DE rent listings; Mietpreisbremse + Mietspiegel as hub rules.',
        evidence: 'src/lib/countries/de.ts',
      },
      energy: {
        score: 96,
        note: 'Energieausweis A+..H parsed onto listing specs + JSON-LD; KfW tier when the copy names it. Never invented.',
        evidence: 'src/lib/countries/de-expose.ts',
      },
      i18n: {
        score: 98,
        note: '10 locales + EUR-native listing/search for DE (no GEL on German exposés); DE hub copy in German.',
      ...
      performance: {
        score: 96,
        note: 'device-budget lite detection (RAM/cores/Save-Data) + capped MapLibre GPU/RAM + MVT tiles (browser never gets national GeoJSON).',
        evidence: 'src/lib/device-budget.ts',
      },
      mapTransit: {
        score: 92,
        note: '18 DE rail systems, 209 Berlin U-Bahn stations, spatial MVT layers (ALKIS footprints, StEP, B-Pläne, parcels).',
      ...
      engineering: {
        score: 99,
        note: '121 green self-checks run on prebuild, repo-weight lock (≈52/96 MB), brand lock, DB-free SSR-safe modules.',
