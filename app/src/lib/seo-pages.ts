...
export function footerKeywordCols(): FooterCol[] {
  ...
  const geoLink = (slug: string[]) => {
    const def = parseSeoSlug(slug)
    if (!def) return null
    const label = def.district
...
export const CITY_PROSE: Record<string, CityProse> = {
  ...
  berlin: {
    ...
    body: [
      ...
      'sivrce-ზე ბერლინის პირველი უბნები უკვე რეგისტრირებულია: მიტე, კროიცბერგი, პრენცლაუერ ბერგი, შარლოტენბურგი, ნოიკოლნი და ფრიდრიხსჰაინი. განცხადებების ინვენტარი ივსება აგენტებთან და დეველოპერებთან პირდაპირი თანამშრომლობით და თბილისის იმავე სტანდარტით გადაიმოწმება.',
    ],
