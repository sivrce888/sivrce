...
if (a.secondary !== '≈ 4 510 080₾') throw new Error(`unexpected secondary: ${a.secondary}`)
...
if (eurUsd.primary !== '€1 518 545') throw new Error(`unexpected EUR primary: ${eurUsd.primary}`)
...
if (eurGel.primary !== '€909') throw new Error(`unexpected EUR primary: ${eurGel.primary}`)
...
// €480 000 × (2.97/2.61) — NOT the $480 000 the old GEL|USD-only engine emitted.
const eurInUsd = formatListingPrice({ ...eurNative, currencyPreference: 'USD' as const })
if (eurInUsd.primary !== '$546 207') throw new Error(`EUR→USD conversion wrong: ${eurInUsd.primary}`)
if (eurInUsd.secondary !== '≈ 1 425 600₾') throw new Error(`EUR→GEL conversion wrong: ${eurInUsd.secondary}`)
...
if (aedUsd.secondary !== `≈ 1 694 989₾`) throw new Error(`AED→GEL wrong: ${aedUsd.secondary}`)

// EUR unit helpers mirror the USD contract.
if (formatMoney(3040, 'EUR', USD_GEL_FALLBACK, EUR_GEL_FALLBACK) !== '€1 024') throw new Error('formatMoney EUR')
if (convertGel(3040, 'EUR', USD_GEL_FALLBACK, EUR_GEL_FALLBACK) !== 1024) throw new Error('convertGel EUR')
if (formatMapPin(3_040_000, 'EUR', USD_GEL_FALLBACK, EUR_GEL_FALLBACK) !== '€1M') throw new Error('formatMapPin EUR M')
if (formatMapPin(91_200, 'EUR', USD_GEL_FALLBACK, EUR_GEL_FALLBACK) !== '€31k') throw new Error('formatMapPin EUR k')
...
if (eurOnDe.secondary !== '≈ €1 518 545') throw new Error(`DE market cross: ${eurOnDe.secondary}`)

// AED unit helpers mirror the EUR contract.
if (formatMoney(3040, 'AED', USD_GEL_FALLBACK) !== 'AED 4 278') throw new Error('formatMoney AED')
if (formatMapPin(3_040_000, 'AED', USD_GEL_FALLBACK) !== 'AED 4.3M') throw new Error('formatMapPin AED M')
