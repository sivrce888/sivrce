...
export const DEVELOPERS_HUB: Record<DirLoc | 'de', DirectoryHubCopy> = {
  ka: {
    ...
    sub: 'ყველა სამშენებლო კომპანია თბილისში, ბათუმსა და მთელ საქართველოში — მიმდინარე პროექტებით, ფასებით, შიდა განვადებითა და მყიდველების მიმოხილვებით',
    proseTitle: 'ბინა დეველოპერისგან — რატომ ღირს',
    prose: [
      'sivrce-ზე შეგროვებულია საქართველოს ყველა მნიშვნელოვანი დეველოპერი: Archi, m², Alliance Group, ORBI, Domus, Blox, Eagle Hills, King David, White Square და სხვა. თითოეული კომპანიის პროფილზე ნახავთ მიმდინარე მშენებლობებს თბილისსა და ბათუმში, ჩაბარებულ პროექტებს, მისამართებს, რუკის კოორდინატებსა და მყიდველების შეფასებებს.',
      'ბინა დეველოპერისგან ყიდვა ნიშნავს საუკეთესო ფასს შუამავლის გარეშე, შიდა განვადებას საწყისი შენატანით და კარკასის არჩევას — შავი, თეთრი ან მწვანე. შეადარეთ კვადრატული მეტრის ფასები, ჩაბარების ვადები და კომპანიების სანდოობა ერთ სივრცეში, სანამ ახალ პროექტში ბინას შეიძენთ.',
    ],
    ...
    faqs: [
      ...
      },
      {
        ...
        a: 'შავი კარკასი — მხოლოდ კონსტრუქცია და კარ-ფანჯარა. თეთრი კარკასი — მზა რემონტისთვის: მუყაოს კედლები, ელექტრო-სანტექნიკა, იატაკის მოსამზადებელი ფენა. მწვანე კარკასი — თითქმის სრული რემონტი, რჩება მხოლოდ ავეჯი. კვადრატული მეტრის ფასი პირდაპირ არის დამოკიდებული კარკასის ტიპზე.',
      },
      {
    ...
    faqs: [
      {
        ...
        a: 'კვადრატული მეტრის ფასი თბილისში დაახლოებით $1,100–1,200-დან იწყება (გლდანი, დიდი დიღომი) და $3,000–4,000-ს აღწევს ვაკეში და მთაწმინდაზე. ბათუმში ზღვისპირა პროექტები $1,500–3,000-ში მერყეობს. ზუსტი ფასები ნახეთ თითოეული პროექტის გვერდზე.',
      },
      {
      ...
      },
      {
        ...
        a: 'მშენებლობის ეტაპზე გასაყიდი ბინა ჩაბარებამდე, როგორც წესი, იზრდება ღირებულებაში; თბილისსა და ბათუმში გაქირავების შემოსავალი (ROI) უბნისა და ტურისტული მოთხოვნის მიხედვით მერყეობს. შეადარეთ პროექტების ფასი, მდებარეობა და დეველოპერის სანდოობა sivrce-ზე.',
      },
    ],
  },
  en: {
...
> = {
  ka: {
    ...
    descFallback: (name, city) =>
      `${name} — დეველოპერი ${cityIn(city, 'ka')}. პროექტები, ფასები და მიმოხილვები სივრცეზე.`,
    crumbHome: 'მთავარი',
...
> = {
  ka: {
    ...
    floorsCaption: (floors, flats, done) =>
      `${floorsLabel(floors, 'ka')} · ${unitsLabel(flats, 'ka')} · აშენებულია ${done}% · გადაატარე მაუსი სართულებზე`,
    ...
    return [
      {
        ...
        a:
          projects.length > 0
            ...
            : `${name} — დეველოპერი ${cityIn(dev.city, 'ka')}. პროექტები ხელმისაწვდომია სივრცეზე.`,
      },
      {
        q: `სად მუშაობს ${name}?`,
        a: `${name} მუშაობს ${cityIn(dev.city, 'ka')}. ${dev.yearsActive > 0 ? `აქტიურია ${dev.yearsActive} წელია. ` : ''}${pickLoc(dev.description, loc).slice(0, 200)}`,
      },
      {
      ...
      },
      {
        ...
        a: `საქართველოში დეველოპერების უმეტესობა გთავაზობს შიდა განვადებას: პირველადი შენატანი ჩვეულებრივ 10–30%, დარჩენილი თანხა — თვიური გადახდებით ჩაბარებამდე, ხშირად 0%-იანი პროცენტით. ${name}-ის ზუსტი პირობები დაადასტურეთ გაყიდვების ოფისთან ან დაგვიკავშირდით sivrce-ზე.`,
      },
      {
...
export const PROJECT_HUBS: Record<ProjectHubKey, Record<DirLoc, DirectoryHubCopy>> = {
  ...
  batumi: {
    ka: {
      ...
      faqs: [
        ...
        {
          q: 'ბათუმის რომელ უბნებში ღირს ბინის ყიდვა?',
        ...
        },
        {
          ...
          a: 'ბათუმში მშენებლობის ეტაპზე გასაყიდი ბინა ჩაბარებამდე, როგორც წესი, იზრდება ღირებულებაში; ზღვისპირა პროექტების სეზონური ქირა ქვეყანაში უმაღლესია. შეადარეთ პროექტების ფასი და მდებარეობა sivrce-ზე.',
        },
      ],
    },
    en: {
    ...
    ka: {
      ...
      faqs: [
        ...
        },
        {
          ...
          a: 'ბათუმის ზღვისპირა ბინების სეზონური მოკლე ვადიანი ქირა ქვეყანაში უმაღლესია — ხედითა და პირველი ხაზით ფასი და დაკავება ორივე იზრდება. შეადარეთ პროექტების მდებარეობა და ფასი sivrce-ზე.',
        },
      ],
    },
    en: {
  ...
  installment: {
    ka: {
      ...
      prose: [
        'შიდა განვადება დეველოპერის საკუთარი დაფინანსებაა ბანკის გარეშე: იხდით პირველად შენატანს — ჩვეულებრივ ღირებულების 10–30%-ს — და დარჩენილ თანხას თვიური გადახდებით ბინის ჩაბარებამდე, ხშირად 0%-იანი პროცენტით. ეს ბინის ყიდვის ყველაზე მარტივი გზაა — იპოთეკის გრძელი პროცედურების გარეშე.',
      ...
      faqs: [
        ...
        {
          q: 'რა ხდება, თუ გადახდას ვერ შევძლებ?',
  ...
  ready: {
    ka: {
      ...
      description:
        'ჩაბარებული ბინები ახალ კორპუსებში თბილისსა და ბათუმში: მზა საცხოვრებლად ან გასაქირავებლად — ფასები კვადრატულზე და მდებარეობები ერთ სივრცეში.',
      ...
      faqs: [
        {
          ...
          a: 'კორპუსი სრულად დასრულებულია და ჩაბარებულია ექსპლუატაციაში — ბინის რეგისტრაცია და გამოყენება დაუყოვნებლივ შეიძლება. ამ სიაში მხოლოდ 100%-ით აშენებული პროექტებია.',
        },
        {
